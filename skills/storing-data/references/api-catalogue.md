...
| `long[] storeAll(Iterable<?> instances)` | object ids | Same, for each element. The iterable itself is not stored. |
...
File: `persistence/base/src/main/java/org/eclipse/serializer/persistence/types/Storer.java`

| Method | Purpose |
|---|---|
| `long store(Object)` | Enqueue (does **not** write yet). |
| `long[] storeAll(Object...)` | Enqueue many. |
| `long[] storeAll(Iterable<?>)` | Enqueue many. |
| `long commit()` | Flush to disk atomically. |
| `void clear()` | Discard enqueued stores without committing. |
| `boolean isEmpty()` | True if nothing is enqueued. |
| `long currentCapacity()` | Current slot capacity (tuning). |
| `Storer reinitialize()` | Reset the internal state for reuse. |

A `Storer` is **not** thread-safe. Use one per thread, or serialize access.

## `BatchStorer`

File: `storage/embedded/src/main/java/org/eclipse/store/storage/embedded/types/BatchStorer.java`

### Builder

Obtain via `storageManager.batchStorerBuilder()`. Methods:

| Method | Default | Notes |
|---|---|---|
| `maxSize(long)` | — | Flush after this many pending objects. |
| `flushCycle(Duration)` | — | Flush after this much time since last flush. |
| `checkInterval(Duration)` | 1 s | How often the daemon checks whether to flush. |
| `build()` | — | Throws `IllegalStateException` if neither `maxSize` nor `flushCycle` set. |
...
| `long[] storeAll(Object...)` | |
| `long[] storeAll(Iterable<?>)` | |
| `void flush()` | Force flush now. |
| `boolean hasPendingData()` | Inspect. |
| `long commit()` | Flush + release. |
...
File: `persistence/binary/.../PersistenceObjectRegistrationListener.java`.
